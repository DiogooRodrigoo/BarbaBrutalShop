...
export default function Sumario() {
  ...
  function renderizarServico(nome: string, i: number) {
    return (
      <div key={1} className="flex items-center bg-zinc-700 rounded-md">
  ...
  return (
    <div className="flex flex-col bg-zinc-950 rounded-lg w-96 lg:w-80">
      ...
      <div className="flex justify-between items-center border-b border-zinc-800 p-5">
        ...
        <span className="text-white font-semibold">R$ {precoTotal()}</span>
      </div>
      ...
      <div className="p-5">
        <button
          ...
          className={`flex justify-center items-center text-sm font-semibold
            ${podeFinalizar() ? "bg-yellow-400" : "bg-zinc-600"} text-zinc-900 w-full py-3 rounded-lg`}
        >
          {carregando && data ? (
