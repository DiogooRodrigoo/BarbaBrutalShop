...
export default function HorariosInput(props: HorariosInputProps) {
  ...
  function renderizarHorario(horario: string) {
    const periodo = obterPeriodo(horaHover, props.quantidadeHorarios);
    const temHorarios = periodo.length === props.quantidadeHorarios;
    const destacarHora = temHorarios && periodo.includes(horario);
    const periodoSelecionado = obterPeriodo(
      horaSelecionada,
      props.quantidadeHorarios
    );
    const selecionado =
      periodoSelecionado.length === props.quantidadeHorarios &&
